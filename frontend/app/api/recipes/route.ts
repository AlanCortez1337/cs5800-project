...
export async function GET(request: NextRequest) {
  ...
  const id = searchParams.get('id');  // Changed from recipeID to id
  
  try {
    let endpoint = `${SPRING_BOOT_API}/api/recipes`;
    
    if (id) {
      endpoint += `/${id}`;  // This builds /api/recipes/1
    }
...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    const { recipeID, ...recipeDetails } = body;
    ...
    const response = await fetch(`${SPRING_BOOT_API}/api/recipes/${recipeID}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
